  ...
  Describe "Move-Commit" {
    ...
    It "can write a reviewable Move-Commit script without executing it" {
      ...
      try {
        ...
        $scriptText | Should -Match ([regex]::Escape("core.longpaths=true"))
        $scriptText | Should -Match ([regex]::Escape('& git @longPathGitArgs worktree add $destWorktreePath $destinationBranch'))
        $scriptText | Should -Match ([regex]::Escape('& git @longPathGitArgs -C $destWorktreePath -c "core.hooksPath=$disabledHooksPath" cherry-pick $commitHash'))
    ...
    It "can generate and execute a Move-Commit script that creates the destination branch" {
      ...
      try {
        ...
        $scriptText | Should -Match ([regex]::Escape('& git @longPathGitArgs worktree add -b $destinationBranch $destWorktreePath $destinationCreateBaseCommit'))
    ...
    It "writes branch resync steps into Move-Commit scripts when removing from source" {
      ...
      try {
        ...
        $scriptText | Should -Match ([regex]::Escape('& git @longPathGitArgs worktree add --detach $sourceWorktreePath $expectedBranch'))
        $scriptText | Should -Match ([regex]::Escape('& git update-ref "refs/heads/$expectedBranch" $rewrittenHead $expectedHead'))
        $scriptText | Should -Match ([regex]::Escape('& git @longPathGitArgs reset --hard "refs/heads/$expectedBranch"'))
      }
      finally {
        Reset-GitSplitTestHooks
        if ($scriptPath -and (Test-Path $scriptPath)) {
